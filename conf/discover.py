...
#_CPU = "/host/cgroup/cpuacct/docker/"
_UPDATE_TIME = 15
_STAT_REGEX = re.compile("\s+([0-9.]+)%\s+([0-9.]+)\s+(\w+)\s+\/\s+([0-9.]+)\s+(\w+)\s+([0-9.]+)%\s+([0-9.]+)\s+(\w+)\s+\/\s+([0-9.]+)\s+(\w+)\s+([0-9.]+)\s+(\w+)\s+\/\s+([0-9.]+)\s+(\w+)(\s+(\d+))?")
_CPU_REGEX = re.compile("cpu\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)")
